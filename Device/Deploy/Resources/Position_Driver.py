...
def on_request(ch, method, properties, body):
    ...
    if (req_type=="get"):
        resp="{'latitude':'52.405259','longitude':'-1.499708','city':'Coventry','country':'United Kingdom'}"
        properties_m=pika.BasicProperties(headers={'app':app_name,'res': "position"})
        channel.basic_publish(exchange='resource', routing_key='', body=resp, properties=properties_m)
    ch.basic_ack(delivery_tag = method.delivery_tag)
channel.basic_consume(on_request, queue=Config.get("Position","queue"))
