...
public class Activator extends Thread implements BundleActivator, ManagedService {
	...
	private static String CONFIG_PID = "org.karaf.broker";
	...
	public class SendEventHandler implements EventHandler {
		...
		public void handleEvent(Event event) {
			...
			for (String device : dev_names)
			{
				if (!device.equals(comm))
				{
					logger.warn("Send to:"+device+"what: "+send);
					deviceSendEvent(send, device);
				}
			}
		}
	...
	public class CloudEventHandler implements EventHandler {
		...
		public void handleEvent(Event event) {
			...
			String send="[{'n':'"+parts[0]+"','v':'"+parts[1]+"'}]";
			logger.warn("Received from Cloud: "+value);
					logger.warn("Send to:"+dev_names.get(0)+"what: "+send);
					deviceSendEvent(send, dev_names.get(0));
			}
	...
	public void updated(Dictionary properties) throws ConfigurationException {
		...
		if (properties != null) {
			...
			if (devs1 != null && devs2 != null && app != null && clouds !=null) {
				reconfigureDev(devs1.trim(),devs2.trim());
	...
	public void reconfigureDev(String dev1, String dev2) {
		if (configured) {
			for (ServiceRegistration reg : dev_reg) {
				reg.unregister();
				dev_names.clear();	
	...
	public void reconfigureApp(String name) {
		...
		Dictionary dp2 = new Hashtable();
		dp2.put(EventConstants.EVENT_TOPIC, CLOUD_QUEUE + app_name);
		logger.warn("New App Name Reg:" + name);
		register2 = bcontext.registerService(EventHandler.class.getName(), new CloudEventHandler(), dp2);
	}
}
