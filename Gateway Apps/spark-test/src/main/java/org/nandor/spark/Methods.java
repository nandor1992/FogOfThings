...
public class Methods {
	...
	public static Fog InitFog(int ClsCount, int cloudGw){
		...
		f.generateNewFog(ClsCount,(float)30,(float)50,(float)0.2,(float)0.05,lat,cloudGw,lat2,lat3,7,1);
	...
	public static List<Map<Integer, Integer>> GAGlobal(Fog f,int size,int cnt,boolean safe){
		...
		long start=System.currentTimeMillis();
		Genetic g = new Genetic(f);
		f.setDeplpyment(g.GAGlobal(size,cnt,safe,1000));
		f.deployFog();
		System.out.println("Unalocated Apps: "+f.checkIfAppsAllocated());	
		System.out.println("Fog Utility: "+f.getFogCompoundUtility());
		System.out.println("Time Elapsed: "+(System.currentTimeMillis()-start)/(float)1000);
	...
	public static List<Map<Integer, Integer>> GAClus(Fog f,int size,int cnt,boolean safe){
		...
		//DisplayData(f);
		for (Integer i: f.getClusters().keySet()){
			f.getClusters().get(i).setDeployment(g.GACluster(size,cnt, f.getClusters().get(i),safe,1000));
		}
		f.deployClusters();
		if (f.checkIfAppsAllocated().size()==0){
			data.put("1.Utility",f.getFogCompoundUtility().toString());
	...
	public static void CorrelationClusterin(Fog f){
		WeightedCls cls = new WeightedCls(f);
		Map<String,Double> corrApp = new java.util.HashMap<>();
		corrApp.put("Constraints", -0.016);corrApp.put("RequirementSim", -0.0024);corrApp.put("ResourceShare", 0.275);
		corrApp.put("UtilityWeights", 0.0);corrApp.put("MessageRate",-0.0167);corrApp.put("Distance", -0.093);corrApp.put("UnitLoad", -0.027);
		Map<String,Double> corrGw = new java.util.HashMap<>();
		corrGw.put("Capabilities", 0.0);corrGw.put("SharedRes", 0.3601);corrGw.put("PerfToULoad", 0.00000528);
		corrGw.put("BaseLoad", 0.004203);corrGw.put("CapToULoad", -0.0076);
		cls.setCorrelation(corrApp,corrGw,0.05,0.1);
		long start = System.currentTimeMillis();
		WeightedClustering(f, cls,(float)1.1, 6,12);
		weightedResourceAlloc(f,cls,3,0.2);
		System.out.println("Clustering Time Elapsed: " + (System.currentTimeMillis() - start) / (float) 1000);
		//Methods.displayClsAndRes(f);
		//System.out.println(cls.getNeighbours(1, (float) 0.7, 4));
	}
